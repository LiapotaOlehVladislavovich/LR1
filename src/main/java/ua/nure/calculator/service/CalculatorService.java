...
public class CalculatorService {
    ...
    private Queue<String> infixToPostfix(String expression) {
        ...
        for (String t : tokens) {
            ...
            if (fixedToken.matches("^-?\\d+$")
                    || fixedToken.matches("^-?\\d*\\.\\d+$")) {
                output.offer(fixedToken);
            } else if (fixedToken.matches("^[a-zA-Z]$")) {
                output.offer(variables.containsKey(fixedToken)
                        ? evaluateExpression(variables.get(fixedToken))
                        : fixedToken);
